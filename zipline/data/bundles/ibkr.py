...
def ib_get_data_for_symbol(symbol, start, end, interval):
    ...
    for i in range(0, len(sessions) + len(sessions) % batch_size, batch_size):
      batch = sessions[i:i + batch_size]
      batch_end = batch[-1].strftime('%Y%m%d 23:59:59')
      batch_duration = str(len(batch))
      print(symbol + ': batch-end: ' + batch_end + ' batch-size:' + batch_duration)
      ...
      bars = CLIENT.reqHistoricalData(contract, endDateTime=batch_end, durationStr=batch_duration + ' D',
      ...
      merged_result = merged_result.append(df)

    #print(merged_result)

    return merged_result.sort_index()
